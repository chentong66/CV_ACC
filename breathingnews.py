...
def main():
    ...
    #t.start()
    f = open('tmp/datat.txt','w')
    list2=[]
    f2 = open("tmp/data.txt","w")
    while not interrupt_handler.got_signal:
        ...
        if plot_data is not None:
            try:
                zoom_hist1=plot_data['zoom_hist1']
                ab=str(zoom_hist1[799])+' '+str(get_simple_time_stamp())+'\n'
                list2.append(str(zoom_hist1[799])+' ')
                list1.append(ab)
...
class BreathingProcessor:
    ...
    def __init__(self, config):
        ...
        self.breath_sos = np.concatenate(butter(2, 2 * 0.01/self.f))
    ...
    def process(self, sweep):
        if self.sweep_index == 0:
            ...
            zoom_hist_plot1 = zoom_hist_plot
